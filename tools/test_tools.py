...
def render_test_sr(position_embedder, view_embedder, model_coarse, model_fine, render_poses, hwf, args, gt_imgs=None,
                gt_labels=None, ins_rgbs=None, savedir=None, matched_file=None):
    H, W, focal = hwf
    ...
    if gt_imgs is not None:
        ...
        mean_output = np.array(
            [np.nanmean(psnrs), np.nanmean(ssims), np.nanmean(lpipses), out_ap[0], out_ap[1], out_ap[2], out_ap[3], out_ap[4], out_ap[5]])
...
def render_test_replica(position_embedder, view_embedder, model_coarse, model_fine, render_poses, hwf, args,
                        gt_imgs=None,
                        gt_labels=None, ins_rgbs=None, savedir=None, matched_file=None):
    H, W, focal = hwf
    gt_imgs_cpu = gt_imgs.cpu().numpy()
    gt_imgs_gpu = gt_imgs.to(args.device)
    lpips_vgg = lpips.LPIPS(net="vgg").to(args.device)
    gt_ins = torch.zeros(size=(H, W, args.ins_num)).cpu()
    _, _, dataset_name, scene_name = args.datadir.split('/')
    ...
    for i, c2w in enumerate(render_poses):
        ...
        if gt_imgs is not None:
            ...
            gt_label_nnnn = valid_gt_labels.cpu().numpy()
            if valid_gt_num > 0:
                # mask = (gt_label < args.ins_num + 1).type(torch.float32)
                pred_label, ap, pred_matched_order = ins_eval(ins.cpu(), gt_ins, valid_gt_num, args.ins_num)
            else:
                pred_label = -1 * torch.ones([H, W])
                ap = torch.tensor([0.0])
...
def render_test_(position_embedder, view_embedder, model_coarse, model_fine, render_poses, hwf, args, gt_imgs=None,
                gt_labels=None, ins_rgbs=None, savedir=None, matched_file=None, crop_mask=None):
    H, W, focal = hwf
    gt_imgs_cpu = gt_imgs.cpu().numpy()
    gt_imgs_gpu = gt_imgs.to(args.device)
    ...
    for i, c2w in enumerate(render_poses):
        ...
        if savedir is not None:
            ...
            gt_img = gt_imgs_cpu[i]
            gt_img_file = os.path.join(savedir, f'{i}_gt.png')
